...
symbols/    – symbolic primitives
corpus/     – example encodings
site/       – public one-page site
```
...
```
⊙(human) Σ(meso)
◯(knowledge_low) → ◯(knowledge_high)
```

**Meaning:**

A human observer at meso scale observes a transition from a lower knowledge state to a higher one.

---

### Example 2 — System load

```
⊙(system_monitor) Σ(system)
◯(stable_state) → ◯(overloaded_state)
```

**Meaning:**

A system transitions from stable operation to an overloaded state.
...
```
⊙(agent) Σ(cognitive)
◯(uncertain_state) → ◯(decision_state)
```

**Meaning:**

An agent transitions from uncertainty to a resolved decision state.
...
Two systems interact and influence each other.
...
Use GitHub Issues:
[https://github.com/13golds/universal-meaning-protocol/issues](https://github.com/13golds/universal-meaning-protocol/issues)
